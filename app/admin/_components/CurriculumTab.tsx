    ...
    items.forEach((item) => {
      fetch("/api/admin/reconcile-topics/suggest", {
        ...
        .then((d: { suggestion?: string; error?: string }) => {
          setCards((prev) => ({
            ...prev,
            [item.id]: { status: "ready", suggestion: d.suggestion ?? null, error: d.error ?? null },
          }));
          if (d.suggestion) setTopicPicker((prev) => ({ ...prev, [item.id]: d.suggestion! }));
        })
        .catch(() => {
          setCards((prev) => ({
            ...prev,
    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 flex-shrink-0"
          ...
          <div>
            <p className="text-sm font-semibold" style={{ color: "var(--text-primary)" }}>Unmatched Topics</p>
            <p className="text-xs mt-0.5" style={{ color: "var(--text-muted)" }}>
              {visible.length} question{visible.length !== 1 ? "s" : ""} with unrecognized topic names — assign a canonical topic or delete
            </p>
          </div>
              ...
              <div key={item.id} className="rounded-xl overflow-hidden"
                ...
                <div className="px-4 py-3" style={{ borderBottom: "1px solid rgba(245,158,11,0.15)" }}>
                  <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                    <span className="text-xs font-semibold" style={{ color: "#f59e0b" }}>Unrecognized:</span>
                  ...
                  {card.status !== "loadingSuggestion" && (
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <button onClick={() => assignTopic(item.id, pickerValue)} disabled={busy || !pickerValue}
                        ...
                        style={{ background: "rgba(74,222,128,0.12)", color: "#4ade80", border: "1px solid rgba(74,222,128,0.3)", opacity: busy ? 0.5 : 1 }}>
                        {card.status === "assigning" ? "Assigning…" : "Assign Topic"}
                      </button>
                      <button onClick={() => deleteQuestion(item.id)} disabled={busy}
                        className="px-3 py-1.5 rounded-lg text-xs font-semibold flex-shrink-0"
                        style={{ background: "rgba(224,92,92,0.1)", color: "#e05c5c", border: "1px solid rgba(224,92,92,0.25)", opacity: busy ? 0.5 : 1 }}>
