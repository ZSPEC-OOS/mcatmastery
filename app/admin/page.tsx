...
function DatabaseTab() {
  ...
  return (
    <div>
      {loading && <p className="text-sm" style={{ color: "var(--text-muted)" }}>Loading stats…</p>}

      {stats && (
        <>
          {/* Section + difficulty stat cards */}
          ...
          <div className="rounded-xl overflow-hidden" style={{ border: "1px solid var(--border)" }}>
            ...
            <div className="hidden md:grid text-xs px-4 py-2"
              ...
              <span>Section</span><span>Stem</span><span>Difficulty</span><span>Date</span><span></span>
            </div>
            ...
            {/* Mobile cards */}
            {stats.recent.map((q) => (
              <div key={`m-${q.id}`} className="md:hidden p-4" style={{ borderTop: "1px solid var(--border)" }}>
