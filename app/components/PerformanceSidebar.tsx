...
const SEED: DashStats = {
  lastFL: 509,
  sectionMap: {
    "Chem/Phys":   { correct: 68, total: 100 },
    "CARS":        { correct: 72, total: 100 },
    "Bio/Biochem": { correct: 74, total: 100 },
    "Psych/Soc":   { correct: 76, total: 100 },
  },
  weakTopics: [
    { topic: "Enzyme Kinetics",     section: "Bio/Biochem", accuracy: 45 },
    { topic: "Kinematics",          section: "Chem/Phys",   accuracy: 52 },
    { topic: "Scientific Reasoning",section: "CARS",        accuracy: 56 },
    { topic: "Identity",            section: "Psych/Soc",   accuracy: 58 },
  ],
  recentAcc: null,
};

export default function PerformanceSidebar() {
  const [stats, setStats] = useState<DashStats>(SEED);

  useEffect(() => {
    fetch("/api/dashboard")
      .then(r => r.json())
      .then((data: DashStats) => { if (data.lastFL !== undefined) setStats(data); })
      .catch(() => {});
  }, []);

  const sparkData = [45, 52, 61, 58, 65, 70, stats.recentAcc ?? 62];
  ...
  const pts = sparkData.map((v, i) => {
    ...
    const y = padY + ((maxV - v) / (maxV - minV)) * (h - padY * 2);
    return `${x},${y}`;
  }).join(" ");
      ...
      <div className="rounded-xl p-5" style={{ background: "var(--bg-card)", border: "1px solid var(--border)" }}>
        ...
        <div className="mb-4">
          ...
          <div className="space-y-1.5">
            {(stats.weakTopics.length > 0 ? stats.weakTopics : SEED.weakTopics).map(t => (
